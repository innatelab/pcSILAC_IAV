...
#SBATCH -J msglm_yhuang_iav_phubel_exp123
#SBATCH --get-user-env
...
${CHUDIS_PATH}/slurmstep_process_chunks.sh "${PROJECT_ID}_phubel_exp123_${FIT_VERSION}" $USER \
"ch-run $IMAGES_PATH/archpc.msglm \
  ...
  -b $SCRATCH:/scratch \
  -- Rscript /projects/adhoc/$PROJECT_ID/msglm_fit_chunk_exp123.R \

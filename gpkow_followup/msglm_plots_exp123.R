...
modelobj_suffix <- "_exp123"

source(file.path(misc_scripts_path, 'ggplot_ext.R'))
source(file.path(misc_scripts_path, 'furrr_utils.R'))

ko_palette <- c("NTC" = "gray", "GPKOW" = "#F9CB40")
treatment_palette <- c("mock" = "gray", "IFN" = "#D9514EFF", "IVT4" = "#2DA8D8FF")
...
#Making boxplot for all proteins. The dots are from the original LFQ intensity values of MQ.----
sel_objects.df <- dplyr::filter(modelobjs_df, str_detect(gene_names, "MX1"))  #This is used for debugging
...
rowwise(sel_objects.df) %>%
  ...
  #future_group_walk(.progress=TRUE, .options=plot_furrr_opts,
                    function(sel_obj.df, ignore.df) {
                      message("Plotting ", sel_obj.df$object_label, " box plots")
                      sel_obj_conds.df <- dplyr::semi_join(dplyr::filter(fit_stats$object_conditions, ci_target == sel_ci_target & str_starts(var, "obj_cond_labu")),
                        ...
                        dplyr::inner_join(dplyr::select(msglm_def$conditions, condition,treatment), by="condition") %>%
                        dplyr::mutate(dplyr::across(c(mean, median, starts_with("q")),
                                                    ~2^(.x + obj_labu_shift)))%>%
                        dplyr::mutate(q97.5_limit = max(median + (q75 - q25)*5))
                      
                      #The following section adds the individual LFQ intensities to the bar plot
                      sel_obj_msdata.df <- sel_obj.df %>%
                        dplyr::inner_join(msdata_full$protein2protregroup, by="protregroup_id") %>%
                        dplyr::inner_join(msdata_full$protein2protgroup, by="protein_ac") %>%
                        dplyr::select(object_id, protgroup_id) %>% dplyr::distinct() %>%
                        dplyr::inner_join(msdata_full$protgroup_intensities, by="protgroup_id") %>%
                        dplyr::right_join(msdata$msruns, by="msexperiment") %>%
                        dplyr::left_join(dplyr::select(msdata$msrun_shifts, msrun, total_msrun_shift), by="msrun") %>%
                        dplyr::mutate(intensity_norm = intensity * 2^(-total_msrun_shift)) #why is it 2^here not exp() like before?
                      ...
                      obj_shifts.df = dplyr::inner_join(sel_obj_msdata.df,
                        ...
                        dplyr::ungroup()
                      sel_obj_msdata.df <- dplyr::left_join(sel_obj_msdata.df, obj_shifts.df, by="object_id") %>%
                        dplyr::mutate(intensity_norm = intensity_norm * 2^(-obj_shift))
                      #print(dplyr::select(sel_obj_conds.df, object_id, object_label, virus, inhibitor, aa, timepoint, mean))
                      ...
                      if (nrow(sel_obj_conds.df) > 0) {
                        obj_plot <-
                          ...
                          scale_y_log10() +
                          ggtitle(str_c(sel_obj.df$object_label, " fit"),
                        ...
                        ggsave(obj_plot, file = file.path(plot_path,
                                                          str_c(project_id, "_", fit_version, "_",
                                                                str_remove(sel_obj.df$object_label[[1]], "\\.\\.\\.$"),
                                                                "_", sel_obj.df$object_id[[1]], ".pdf")),
                               width = 6, height = 6, device = cairo_pdf, family=base_font_family)
                      }
                    })
...
sel_pepmod_intens.df <- tidyr::crossing(pepmodstate_id = sel_pepmodstates.df$pepmodstate_id,
  ...
  dplyr::inner_join(dplyr::select(sel_pepmodstates.df, object_id, object_label, 
                      ...
                      dplyr::distinct()) %>%
  dplyr::inner_join(distinct(select(msdata$msruns, msexperiment, msfraction, msrun, condition, ko, treatment, replicate)) %>%
                      dplyr::left_join(dplyr::select(msdata$msrun_shifts, msrun, total_msrun_shift), by="msrun") %>%
